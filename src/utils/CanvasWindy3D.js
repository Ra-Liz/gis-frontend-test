...
export default class CanvasWindy3D {
  ...
  _tomap(lng, lat, particle) {
    ...
    var pos = Cesium.SceneTransforms.worldToWindowCoordinates(
      this._viewer.scene,
      ct3
    );
    if (!isVisible) {
    ...
    // if (!isVisible) {
    //   particle.occlusionCount = (particle.occlusionCount || 0) + 1;
    //   if (particle.occlusionCount > 3) {
    //     particle.age = 0;
    //   }
    // } else {
    //   particle.occlusionCount = 0;
    // }
  ...
  _tomap1(lng, lat, particle) {
    ...
    // omg
    if (!ct3 || isNaN(ct3.x) || isNaN(ct3.y) || isNaN(ct3.z)) {
      return null;
    }
    ...
    return pos && !isNaN(pos.x) && !isNaN(pos.y) ? [pos.x, pos.y] : null;
  }
  ...
  _getIn(x, y) {
    if (
      x < 0 ||
      x >= this._windData.cols - 1 ||
      y < 0 ||
      y >= this._windData.rows - 1
    ) {
      return [0, 0, 0, 0];
    }
    ...
    const uvw00 = this._grid[y0][x0];
    const uvw10 = this._grid[y0][x1];
    const uvw01 = this._grid[y1][x0];
    const uvw11 = this._grid[y1][x1];

    const uvw = [0, 0, 0, 0];
    for (let i = 0; i < 4; i++) {
      const a = uvw00[i] * (1 - tx) + uvw10[i] * tx;
      const b = uvw01[i] * (1 - tx) + uvw11[i] * tx;
      uvw[i] = a * (1 - ty) + b * ty;
    }
